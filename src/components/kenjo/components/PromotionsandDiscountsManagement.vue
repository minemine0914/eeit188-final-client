    ...
    <v-container fluid>
        <v-toolbar flat color="brown-lighten-5" rounded="lg">
            ...
            <v-sheet class="d-flex flex-row ga-3 mr-3" color="transparent">
                <v-btn
                    color="primary"
                    @click="openCreatePromotionDialog"
                    size="large"
                    variant="tonal"
                    >新增促銷活動</v-btn
                >
                <v-btn
                    color="primary"
                    @click="openCreateDiscountCodeDialog"
                    size="large"
                    variant="tonal"
                    >新增優惠碼</v-btn
                >
        ...
        <v-dialog v-model="dialogPromotion" max-width="500px">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-text-field
                        v-model="currentPromotion.discountRate"
        ...
        <v-dialog v-model="dialogDiscountCode" max-width="500px">
            <v-card>
                ...
                <v-card-text>
                    <v-text-field
                        v-model="currentDiscountCode.name"
                        label="活動名稱"
                    ></v-text-field>
                    <v-text-field
                        v-model="currentDiscountCode.discount"
                        label="折扣金額"
                        type="number"
                    ></v-text-field>
...
const fetchPromotions = async () => {
    try {
        ...
        promotions.value = response.data.content
            .filter((promotion) => promotion.discountRate)
            .map((promotion) => ({
                ...promotion,
...
const fetchDiscountCodes = async () => {
    try {
        const response = await axios.get("/coupon/all");
        discountCodes.value = response.data.content
            .filter((discountCode) => discountCode.discount)
            .map((discountCode) => ({
                ...discountCode,
